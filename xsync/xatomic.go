...
type XAtomicMod[Elem any] interface {
	Set(Elem)
	Get() Elem
	CompareAndSwap(Elem, Elem) bool
}

type NumLike[Elem int32 | int64 | float64 | time.Duration] interface {
	Add(Elem) Elem
	XAtomicMod[Elem]
}
...
func NewAtomicInt32(n int32) NumLike[int32] {
...
func NewAtomicInt64(n int64) NumLike[int64] {
...
func NewAtomicDuration(duration time.Duration) NumLike[time.Duration] {
...
func NewAtomicBool(n bool) XAtomicMod[bool] {
	if n {
		return &AtomicBool{1}
	}
	return &AtomicBool{0}
...
func NewAtomicString(str string) XAtomicMod[string] {
	return &AtomicString{
		str: str,
		mu:  sync.Mutex{},
	}
